    ...
    <div style={{ position:"fixed", inset:0, background:"#0D0F1A", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, zIndex:200 }}>
      ...
      <div style={{ width:"100%", maxWidth:380, position:"relative" }}>
        ...
        <div style={{ background:"#161929", border:"1.5px solid #252A3D", borderRadius:20, padding:24 }}>
          {step === "name" ? (
            <>
              <label style={{ display:"block", fontSize:11, fontWeight:800, letterSpacing:"0.04em", color:"#ADFF4F", marginBottom:10 }}>WHAT'S YOUR NAME?</label>
              ...
              <button onClick={submitName} style={{ width:"100%", background:"#ADFF4F", color:"#0D0F1A", border:"none", borderRadius:12, padding:"14px 0", fontSize:16, fontWeight:800, cursor:"pointer" }}>
                Next →
              </button>
            </>
          ) : (
            <>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
                <div style={{ width:36, height:36, borderRadius:"50%", background:"linear-gradient(135deg,#ADFF4F,#4FC3F7)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:800, color:"#0D0F1A", flexShrink:0 }}>
              ...
              <label style={{ display:"block", fontSize:11, fontWeight:800, letterSpacing:"0.04em", color:"#ADFF4F", marginBottom:10 }}>GROUP PASSWORD</label>
              ...
              <button onClick={submitPassword} disabled={loading}
                style={{ width:"100%", background:"#ADFF4F", color:"#0D0F1A", border:"none", borderRadius:12, padding:"14px 0", fontSize:16, fontWeight:800, cursor:loading?"not-allowed":"pointer", opacity:loading?0.7:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:100, backdropFilter:"blur(6px)" }} onClick={onClose}>
      <div style={{ background:"#161929", border:`1.5px solid ${accent}44`, borderRadius:"20px 20px 0 0", padding:24, width:"100%", maxWidth:500, boxShadow:`0 -8px 40px ${accent}22`, paddingBottom:"max(24px,env(safe-area-inset-bottom))" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={attempt} disabled={loading} style={{ flex:2, background:accent, color:"#0D0F1A", border:"none", borderRadius:12, padding:"13px 0", fontSize:15, fontWeight:800, cursor:"pointer", opacity:loading?0.7:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
  ...
  return (
    <div style={{ background:"#161929", border:`1.5px solid ${color}44`, borderRadius:50, padding:"5px 12px 5px 8px", display:"flex", alignItems:"center", gap:7 }}>
      <div style={{ position:"relative", width:28, height:28, flexShrink:0 }}>
        ...
        <span style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color }}>{balance}</span>
      </div>
...
function CreateForm({ username, isAnon, password, onCreated, showToast }) {
  ...
  const L = { display:"block", fontSize:11, fontWeight:800, letterSpacing:"0.04em", color:"#ADFF4F", marginBottom:8 };
  return (
    <div>
      <h2 style={{ fontSize:24, fontWeight:800, color:"#fff", marginBottom:4, letterSpacing:"-0.5px" }}>Create a Market</h2>
      ...
      <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:isAnon?"rgba(251,146,60,0.08)":"#161929", border:`1px solid ${isAnon?"rgba(251,146,60,0.3)":"#1E2438"}`, borderRadius:8, padding:"7px 14px", marginBottom:20 }}>
        ...
        {isAnon && <span style={{ fontSize:10, fontWeight:800, color:"#FB923C", background:"rgba(251,146,60,0.15)", border:"1px solid rgba(251,146,60,0.3)", borderRadius:4, padding:"2px 6px", letterSpacing:"0.03em" }}>ANON</span>}
      </div>
      <div style={{ background:"#161929", border:"1.5px solid #252A3D", borderRadius:20, padding:20, display:"flex", flexDirection:"column", gap:20 }}>
            ...
            {form.options.map((opt,i) => (
              <div key={i} style={{ display:"flex", gap:8 }}>
                ...
                {mtype==="multi" && form.options.length>2 && (
                  <button onClick={() => setOpts(form.options.filter((_,j)=>j!==i))} style={{ background:"#FF6B6B18", border:"1px solid #FF6B6B44", color:"#FF6B6B", borderRadius:10, padding:"0 14px", fontSize:18, cursor:"pointer", flexShrink:0, fontWeight:800 }}>×</button>
                )}
              </div>
            ))}
            {mtype==="multi" && (
        ...
        <button onClick={submit} disabled={loading} style={{ background:"#ADFF4F", color:"#0D0F1A", border:"none", borderRadius:12, padding:"15px 0", fontSize:16, fontWeight:800, cursor:loading?"not-allowed":"pointer", opacity:loading?0.7:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
...
function SettingsPanel({ settings, adminPassword, showToast, onSaved }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize:24, fontWeight:800, color:"#fff", marginBottom:4 }}>Settings</h2>
      ...
      <div style={{ background:"#161929", border:"1.5px solid #252A3D", borderRadius:20, padding:20, display:"flex", flexDirection:"column", gap:18 }}>
        ...
        <div style={{ borderTop:"1.5px solid #252A3D", paddingTop:18 }}>
          <p style={{ fontSize:14, fontWeight:800, color:"#fff", marginBottom:14 }}>🔐 Passwords</p>
        ...
        <button onClick={save} disabled={loading} style={{ background:"#ADFF4F", color:"#0D0F1A", border:"none", borderRadius:12, padding:"14px 0", fontSize:16, fontWeight:800, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, opacity:loading?0.7:1 }}>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:isAnon?T.bg:T.bg, color:"#fff", paddingBottom:80 }} className={isAnon?"anon-app":""}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
        ...
        /* ── Anon theme overrides ── */
        .anon-app { background:#0E0812 !important; }
        .anon-app header { background:rgba(14,8,18,0.95) !important; border-bottom-color:#2D1F3D !important; }
        .anon-app nav { background:rgba(14,8,18,0.96) !important; border-top-color:#2D1F3D !important; }
      ...
      {/* ── Header ── */}
      <header style={{ position:"sticky", top:0, zIndex:50, background:"rgba(13,15,26,0.95)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${T.border}`, padding:"12px 16px" }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
            ...
            <span style={{ fontSize:17, fontWeight:800, letterSpacing:"-0.5px" }}>Poly<span style={{ color:T.accent }}>frens</span></span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            {role ? (
              <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                <span style={{ fontSize:10, fontWeight:800, letterSpacing:"0.05em", padding:"4px 9px", borderRadius:50, color:role==="admin"?T.accent:"#4FC3F7", background:role==="admin"?"#ADFF4F18":"#4FC3F718", border:`1px solid ${role==="admin"?"#ADFF4F44":"#4FC3F744"}` }}>
      ...
      {/* ── Anon mode banner ── */}
      {isAnon && (
        <div style={{ background:`linear-gradient(90deg,${T.accent}22,${T.accent}0A)`, borderBottom:`1px solid ${T.accentBd}`, padding:"8px 16px", display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
          <span style={{ fontSize:14 }}>🕵️</span>
          <span style={{ fontSize:12, fontWeight:800, color:"#ADFF4F", letterSpacing:"0.03em" }}>ANONYMOUS MODE</span>
          <span style={{ fontSize:12, color:`${T.accent}99` }}>— betting as <strong style={{ color:T.accent }}>{anonAlias}</strong></span>
          <button onClick={toggleAnon} style={{ marginLeft:8, background:"transparent", border:`1px solid ${T.accentBd}`, color:"#ADFF4F", borderRadius:6, padding:"2px 10px", fontSize:11, fontWeight:800, cursor:"pointer" }}>Turn off</button>
        </div>
      )}

      {/* ── Main ── */}
      <main style={{ maxWidth:900, margin:"0 auto", padding:"20px 16px", position:"relative", zIndex:1, background:T.bg }}>
        {loading ? (
              ...
              <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:18 }}>
                  {[["🟢",events.filter(e=>!e.resolved).length,"Live"],["💰",events.reduce((s,e)=>s+getPool(e.bets),0),"Volume"],["✅",events.filter(e=>e.resolved).length,"Done"]].map(([icon,val,label]) => (
                    <div key={label} style={{ background:"#161929", border:`1.5px solid ${T.border}`, borderRadius:12, padding:"10px 12px" }}>
                ...
                <div style={{ display:"flex", gap:6, marginBottom:16 }}>
                  {[["live","🔥 Live",T.accent],["resolved","✅ Done","#C084FC"],["all","All","#94A3B8"]].map(([f,lbl,c]) => (
                    <button key={f} onClick={() => setFilter(f)} style={{ background:filter===f?`${c}18`:"transparent", border:`1.5px solid ${filter===f?c:T.border}`, color:filter===f?c:"#3A4155", borderRadius:50, padding:"6px 0", fontSize:13, fontWeight:800, cursor:"pointer", flex:1, transition:"all .15s" }}>{lbl}</button>
                  ))}
                </div>
                ...
                {filteredEvents.length===0 ? (
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", gap:10 }}>
                    {filteredEvents.map((event, ei) => {
                      ...
                      return (
                        <div key={event.id} style={{ background:"#161929", borderRadius:16, border:`1.5px solid ${canBet?accent+"33":T.border}`, overflow:"hidden", display:"flex", flexDirection:"column", cursor:"default" }}>
                          {/* Accent bar */}
                          ...
                          <div style={{ padding:"14px 14px 12px", flex:1, display:"flex", flexDirection:"column", gap:10 }}>
                            ...
                            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:6 }}>
                              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                                {canBet && (
                                  <span style={{ display:"inline-flex", alignItems:"center", gap:4, background:"#22c55e18", border:"1px solid #22c55e33", borderRadius:50, padding:"2px 8px", fontSize:11, color:"#22c55e", fontWeight:600, whiteSpace:"nowrap" }}>
                                    <span style={{ width:5, height:5, borderRadius:"50%", background:T.dot, animation:"pulse 1.5s infinite", flexShrink:0 }} />
                            ...
                            <div style={{ display:"flex", flexDirection:"column", gap:5, flex:1 }}>
                              {previewOpts.map((opt,i) => {
                                ...
                                return (
                                  <div key={i} style={{ background:isWinner?`${T.accent}0A`:T.bg, border:`1px solid ${isWinner?T.accentBd:T.border}`, borderRadius:9, padding:"7px 10px", opacity:isLoser?0.3:1, display:"flex", alignItems:"center", gap:8 }}>
                                    {/* % */}
                                    <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:13, fontWeight:700, color:isWinner?T.accent:col, minWidth:36, flexShrink:0 }}>{odds[i]}%</span>
                                    {/* Label + bar */}
                                    <div style={{ flex:1, minWidth:0 }}>
                                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:4, marginBottom:4 }}>
                                        <span style={{ fontSize:12, fontWeight:700, color:isWinner?T.accent:"#CBD5E1", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                                          {isWinner&&"✓ "}{opt}
                                        </span>
                                        <div style={{ display:"flex", alignItems:"center", gap:4, flexShrink:0 }}>
                                          {!event.resolved && <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, fontWeight:700, color:isHot?"#FB923C":"#3A4155", background:isHot?"#FB923C18":"transparent", border:isHot?"1px solid #FB923C44":"none", borderRadius:4, padding:isHot?"1px 5px":"0" }}>{multLabel}</span>}
                            ...
                            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:6, borderTop:`1px solid ${T.border}` }}>
                              ...
                              {role==="admin" && !event.resolved && (
                                <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                                  <span style={{ fontSize:11, color:"#5A6478", fontWeight:500 }}>Resolve:</span>
                ...
                {leaderboard().length===0 ? (
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {leaderboard().map((p,i) => {
                      ...
                      return (
                        <div key={p.name} style={{ background:isMe?"#4FC3F708":T.surface, border:`1.5px solid ${isMe?"#4FC3F744":T.border}`, borderRadius:14, padding:"14px 16px", display:"flex", alignItems:"center", gap:14 }}>
                          <span style={{ fontSize:20, minWidth:30, textAlign:"center" }}>{medals[i]||i+1}</span>
                          <div style={{ flex:1, minWidth:0 }}>
                            <p style={{ fontWeight:600, fontSize:15, color:isMe?"#4FC3F7":"#F0F4FF", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.name}{isMe&&" (you)"}</p>
      ...
      {/* ── Bottom nav ── */}
      <nav style={{ position:"fixed", bottom:0, left:0, right:0, background:"rgba(13,15,26,0.96)", backdropFilter:"blur(12px)", borderTop:`1px solid ${T.border}`, padding:"8px 0 max(10px,env(safe-area-inset-bottom))", zIndex:50 }}>
        <div style={{ maxWidth:600, margin:"0 auto", display:"flex", padding:"0 8px" }}>
          {NAV.map(v => (
            <button key={v} onClick={() => setView(v)} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3, background:"transparent", border:"none", cursor:"pointer", padding:"4px 0", transition:"all .15s" }}>
              ...
              <span style={{ fontSize:10, fontWeight:600, letterSpacing:"0.04em", color:view===v?T.accent:"#3A4155" }}>
      ...
      {betModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:100, backdropFilter:"blur(8px)" }} onClick={() => setBetModal(null)}>
          <div style={{ background:"#161929", border:`1.5px solid ${betModal.color}44`, borderRadius:"20px 20px 0 0", padding:24, width:"100%", maxWidth:500, boxShadow:`0 -8px 40px ${betModal.color}22`, paddingBottom:"max(24px,env(safe-area-inset-bottom))" }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize:10, fontWeight:600, letterSpacing:"0.04em", color:betModal.color, marginBottom:4 }}>PLACE BET</p>
            <p style={{ fontWeight:800, fontSize:16, color:"#fff", marginBottom:8, lineHeight:1.3 }}>{betModal.eventTitle}</p>
            ...
            {/* Posting as row */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#0D0F1A", border:`1px solid ${isAnon?"rgba(251,146,60,0.3)":T.border}`, borderRadius:10, padding:"9px 14px", marginBottom:10 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <button onClick={toggleAnon} style={{ background:isAnon?"rgba(251,146,60,0.12)":T.surface, border:`1px solid ${isAnon?"rgba(251,146,60,0.3)":T.border2}`, color:isAnon?"#FB923C":"#5A6478", borderRadius:6, padding:"3px 8px", fontSize:10, fontWeight:800, cursor:"pointer", letterSpacing:"0.05em" }}>
            ...
            <p style={{ fontSize:11, fontWeight:800, letterSpacing:"0.04em", color:"#3A4155", marginBottom:8 }}>AMOUNT ({settings.min_bet}–{Math.min(Number(settings.max_bet), balance||100)} {settings.currency})</p>
            <input autoFocus type="number" placeholder={`Min ${settings.min_bet}`} value={betAmount}
              ...
              style={{ width:"100%", background:"#0D0F1A", border:"1.5px solid #252A3D", color:"#fff", borderRadius:12, padding:"13px 16px", fontFamily:"'JetBrains Mono',monospace", fontSize:18, outline:"none", marginBottom:10 }} />
            {/* Live payout preview */}
            {betAmount > 0 && (() => {
              const amt = Number(betAmount);
              ...
              return (
                <div style={{ background:"#0D0F1A", border:`1.5px solid ${betModal.color}44`, borderRadius:12, padding:"12px 14px", marginBottom:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <p style={{ fontSize:11, fontWeight:800, color:"#3A4155", letterSpacing:"0.04em", marginBottom:3 }}>IF YOU WIN</p>
                    <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:20, fontWeight:700, color:T.accent }}>+{payout} <span style={{ fontSize:13, color:"#5A6478" }}>{settings.currency}</span></p>
                  </div>
                  <div style={{ textAlign:"right" }}>
                    <p style={{ fontSize:11, fontWeight:800, color:"#3A4155", letterSpacing:"0.04em", marginBottom:3 }}>MULTIPLIER</p>
                    <p style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:20, fontWeight:700, color: Number(mult) >= 3 ? "#FB923C" : betModal.color }}>{mult}x</p>
                  </div>
                </div>
              );
            })()}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6, marginBottom:18 }}>
              {[
                ...
                <button key={v} onClick={() => setBetAmount(String(v))} className="bp"
                  style={{ background:betAmount===String(v)?`${betModal.color}22`:T.bg, border:`1.5px solid ${betAmount===String(v)?betModal.color:T.border}`, color:betAmount===String(v)?betModal.color:"#3A4155", borderRadius:10, padding:"10px 0", fontSize:13, cursor:"pointer", fontFamily:"'JetBrains Mono',monospace", fontWeight:700 }}>
                  {v}
                </button>
              ))}
            </div>
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button onClick={placeBet} disabled={betLoading} className="bp"
                style={{ flex:2, background:betModal.color, color:T.bg, border:"none", borderRadius:12, padding:"14px 0", fontSize:16, fontWeight:800, cursor:"pointer", opacity:betLoading?0.7:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                {betLoading ? <><Spinner color={T.bg} /> Locking...</> : "Confirm 🔒"}
              </button>
            </div>
          </div>
        </div>
      ...
      {resolveModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:100, backdropFilter:"blur(8px)" }} onClick={() => setResolveModal(null)}>
          <div style={{ background:"#161929", border:"1.5px solid #C084FC44", borderRadius:"20px 20px 0 0", padding:24, width:"100%", maxWidth:500, paddingBottom:"max(24px,env(safe-area-inset-bottom))" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background:"#ADFF4F0C", border:"1.5px solid #ADFF4F44", borderRadius:12, padding:"14px 16px", fontSize:16, color:"#ADFF4F", fontWeight:800, marginBottom:12 }}>🏆 Winner: {resolveModal.optionLabel}</div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button onClick={resolveEvent} className="bp" style={{ flex:2, background:"#ADFF4F", color:T.bg, border:"none", borderRadius:12, padding:"14px 0", fontSize:16, fontWeight:800, cursor:"pointer" }}>Confirm ✓</button>
            </div>
          </div>
        </div>
      )}
